...
namespace OPAS2.Api
{
  public class GRApiController : BaseApiController
  {
    ...
    public IHttpActionResult ExamineFlowAction()
    {
      ...
      gr = db.goodsReceivings.Find(bizObj.goodsReceivingId);
      if (!string.IsNullOrWhiteSpace(bizObj.remarkOfAprrover))
        gr.remarkOfAprrovers = gr.remarkOfAprrovers + Environment.NewLine +
            bizObj.remarkOfAprrover + " - [" + userDTO.name + "/" +
            userDTO.englishName + "] " + DateTime.Now.ToString();
      db.SaveChanges();

      decimal totalAmountInRMB = 0;
      gr.details.ForEach(detail => { totalAmountInRMB += detail.amountInRMB; });
      #endregion
